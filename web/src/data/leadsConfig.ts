/**
 * Web App URL for the Google Apps Script that logs valuation-form
 * submissions into the "Nadlanzebait — Leads" Google Sheet. See
 * ../../APPS_SCRIPT_SETUP.md for how to get this URL.
 *
 * Left empty, submissions just aren't logged anywhere — the form still
 * opens WhatsApp exactly as before, nothing breaks.
 */
export const LEADS_WEBHOOK_URL =
  "https://script.google.com/macros/s/AKfycbz8c6tJNqwPioyUcwJHqmvcTQxOZuUVTZnmgBvMm1aVaKZXnzjun3XUbJqGNdu0iEjlkA/exec"
